...
class db_class {
    ...
    public function open() {
        $con=mysqli_connect("localhost", "root", "", "wirelev6_igeek_cms");
        return $con;
    }

    public function pdocon() {
        $db=new PDO("mysql:host=localhost;dbname=wirelev6_igeek_cms;", "root", "");
    ...
    function baseUrl($suffix='') {
        ...
        $web_root=$protocol . $_SERVER['HTTP_HOST'] . "/" . "wirelessgeeks/demo_site/cms-admin/";
        $suffix=ltrim($suffix, '/');
    ...
    function LbaseUrl($suffix='') {
        $protocol=strpos($_SERVER['SERVER_SIGNATURE'], '443') !== false ? 'https://' : 'http://';
        $web_root=$protocol . $_SERVER['HTTP_HOST'] . "/" . "wirelessgeeks/demo_site/";
        $suffix=ltrim($suffix, '/');
        return $web_root . trim($suffix);
    }
    ...
    public function Error($msg, $location) {
        $errmsg_arr[]='<script src="https://ajax.googleapis.com/ajax/libs/jquery/1.12.0/jquery.min.js"></script><script>
            $(document).ready(function () {
                setTimeout(function () {
                    $(".alert-box").hide("slow");
                }, 3000);
            });
        </script><link href="' . $this->LbaseUrl() . 'box.css" rel="stylesheet" type="text/css"><div class="alert-box error"><span>error: </span> ' . $msg . '</div>';
        $error_flag=true;
        if ($error_flag) {
    ...
    public function Success($msg, $location) {
        $errmsg_arr[]='<script src="https://ajax.googleapis.com/ajax/libs/jquery/1.12.0/jquery.min.js"></script><script>
            $(document).ready(function () {
                setTimeout(function () {
                    $(".alert-box").hide("slow");
        ...
        </script><link href="' . $this->LbaseUrl() . 'box.css" rel="stylesheet" type="text/css"><div class="alert-box success"><span>success: </span> ' . $msg . '</div>';
        $error_flag=true;
        if ($error_flag) {
            $_SESSION['ERRMSG_ARR']=$errmsg_arr;
            session_write_close();
