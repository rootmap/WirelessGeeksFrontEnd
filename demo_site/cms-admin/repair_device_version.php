...
$table="repair_device_version";
?>
<?php
if (isset($_POST['create'])) {
    extract($_POST);
    if (!empty($repair_device) && !empty($name)) {
        ...
        $insert=array('repair_device'=>$repair_device, 'name'=>$name, 'heading'=>$heading, 'device_photo'=>$device_photo, 'photo_position'=>$photo_position, 'device_detail'=>$device_detail, 'priority'=>$priority, 'date'=>date('Y-m-d'), 'status'=>1);
    ...
    if (!empty($repair_device) && !empty($name)) {
        ...
        $upd2=array('repair_device'=>$repair_device, 'name'=>$name, 'heading'=>$heading, 'device_photo'=>$device_photo, 'photo_position'=>$photo_position, 'device_detail'=>$device_detail, 'priority'=>$priority, 'date'=>date('Y-m-d'), 'status'=>1);
...
<!--[if !IE]><!--><html><!-- <![endif]-->
    ...
    <body class="">
        ...
        <div id="content">
            <h1 class="content-heading bg-white border-bottom">Repair Device Version</h1>
            <div class="innerAll bg-white border-bottom">
                <ul class="menubar">
                    <li class="active"><a href="#">Create New Repair Device Version</a></li>
                    <li><a href="repair_device_version_data.php">Repair Device Version List</a></li>
                </ul>
            </div>
                ...
                <div class="widget widget-inverse" >
                    ...
                    if (isset($_GET['edit'])) {
                        ...
                        <div class="widget-head">
                            <h4 class="heading">Update/Change - Repair Device Version</h4>
                        </div>
                        <!-- // Widget heading END -->

                        <div class="widget-body"><form enctype='multipart/form-data' class="form-horizontal" method="post" action="" role="form">
                                <input type="hidden" name="id" value="<?php echo $_GET['edit']; ?>"><div class='form-group'>
                                    <label  for="inputEmail3" class="col-sm-2 control-label"> Repair Device </label>

                                    <div class='col-sm-6'>
                                        <?php $repair_device=$obj->SelectAllByVal($table, "id", $_GET['edit'], "repair_device"); ?>
                                        <select name="repair_device" class='form-control'>
                                            <option value="">Select A Repair Device</option>
                                            <?php
                                            $sqlrepair_device=$obj->FlyQuery("SELECT id,name FROM repair_devices");
                                            if (!empty($sqlrepair_device)) {
                                                foreach ($sqlrepair_device as $device):
                                                    ?>
                                                    <option <?php if ($repair_device == $device->id) { ?> selected="selected" <?php } ?> value="<?php echo $device->id; ?>"><?php echo $device->name; ?></option>
                                                    <?php
                                                endforeach;
                                            }
                                            ?>
                                    ...
                                    <div class='col-sm-9'>
                                        <textarea id='form-field-1' name='heading' placeholder='Heading' class='form-control'><?php echo $obj->SelectAllByVal($table, "id", $_GET['edit'], "heading"); ?></textarea>
                                    </div>
                                </div><div class='form-group'>
                    ...
                    <?php }else { ?>
                        <!-- Widget heading -->
                        <div class="widget-head">
                            <h4 class="heading">Create New Repair Device Version</h4>
                        </div>
                        <!-- // Widget heading END -->
                        ...
                        <div class="widget-body"><form enctype='multipart/form-data' class="form-horizontal" method="post" action="" role="form"><div class='form-group'>
                                    ...
                                    <div class='col-sm-9'>
                                        <textarea id='form-field-1' name='heading' placeholder='Heading' class='form-control'></textarea>
                                    </div>
                                </div><div class='form-group'>
                                    <label  for="inputEmail3" class="col-sm-2 control-label"> Device Photo </label>
